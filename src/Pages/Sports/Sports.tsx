...
import { fetchSportsNews } from "./SportsSlice";
...
function Sports(): JSX.Element {
  ...
  const sportsNews = useSelector(
    (state: RootState) => state.sportsNews.news.articles
  );
  const isLoading = useSelector((state: RootState) => state.sportsNews.loading);
  const isError = useSelector((state: RootState) => state.sportsNews.error);
  const dispatch: AppDispatch = useDispatch();

  let firstNews = sportsNews?.slice(0, 1)[0];

  useEffect(() => {
    dispatch(fetchSportsNews("7363efcded9848a9868c1228f623bbca"));
        ...
        {isLoading === true ? (
          ...
          <div className="pb-10">
            ...
            <div className="flex justify-center ">
              {sportsNews?.length < showMoreCount ? (
